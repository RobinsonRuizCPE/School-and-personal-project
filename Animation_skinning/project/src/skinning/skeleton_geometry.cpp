...
skeleton_geometry local_to_global(skeleton_geometry const& sk_local,skeleton_parent_id const& parent_id)
{
    ...
    //I DID: Completez la structure sk_global en exprimant les reperes dans les coordonnees globales.

    for(int i=0;i<N;i++)
    {
        if(parent_id[i] == -1)
        {
          sk_global.push_back(sk_local[i]);
        }
        else
        {
            skeleton_joint temp;
            temp.orientation = sk_global[parent_id[i]].orientation * sk_local[i].orientation;
            temp.position = sk_global[parent_id[i]].orientation * sk_local[i].position + sk_global[parent_id[i]].position;
            sk_global.push_back(temp);
        }
    }
...
skeleton_geometry inversed(skeleton_geometry const& skeleton)
{
    ...
    for(skeleton_joint const& joint : skeleton)
    {
        ...
        skeleton_joint temp;
        temp.position = -joint.position;
        temp.orientation = conjugated(joint.orientation);
        sk_inversed.push_back(temp);
    }
...
skeleton_geometry multiply(skeleton_geometry const& skeleton_1,skeleton_geometry const& skeleton_2)
{
    ...
    for(int k=0 ; k<N_joint ; ++k)
    {
        ...
        skeleton_joint temp;
        temp.position = skeleton_1[k].orientation * skeleton_2[k].position + skeleton_1[k].position;
        temp.orientation = skeleton_1[k].orientation*skeleton_2[k].orientation;
        sk.push_back(temp);
...
std::vector<vec3> extract_bones(skeleton_geometry const& skeleton,skeleton_parent_id const& parent_id)
{
    ...
    for(int k=1 ; k<N_joint ; ++k)
    {
        ...
        // completez la structure position avec la position des extermitees des os.
...
skeleton_geometry interpolated(skeleton_geometry const& skeleton_1,skeleton_geometry const& skeleton_2,float const alpha)
{
    ...
    for(int k=0 ; k<N_joint ; ++k)
    {
        ...
        //completez la methode d'interpolation entre les reperes des joints du squelette 1 et du squelette 2
        temp.position = (1.0 - alpha) * joint_1.position + alpha * joint_2.position;
        temp.orientation = slerp(joint_1.orientation,joint_2.orientation,alpha);
        sk.push_back(temp);
    }
