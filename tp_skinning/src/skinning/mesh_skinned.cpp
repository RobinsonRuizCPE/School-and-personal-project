...
void mesh_skinned::apply_skinning(skeleton_geometry const& skeleton)
{
    ...
    for(int k_vertex=0 ; k_vertex<N_vertex ; ++k_vertex)
    {
        ...
        //  vertex(k_vertex) = ...
        vec3 p_final;
        for(skinning_weight w : vertex_weight_data[k_vertex])
        {
            p_final += w.weight *(skeleton[w.joint_id].orientation * vertex_original(k_vertex) + skeleton[w.joint_id].position);
        }
